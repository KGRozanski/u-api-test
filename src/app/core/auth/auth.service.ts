...
export class AuthService {
            ...
            this.sendRefreshReq(TokenType.ACCESS_TOKEN).subscribe({
                ...
                complete: async () => {
                    ...
                    try {
                        ...
                        if (
                            CURRENT_OIDC_TIMEOUT > 18000 &&
                            !(await this.doesUserUpdatedAccountInfo(this.AccountInfo))
                        ) {
            ...
            this.sendRefreshReq(type)
                ...
                .subscribe({
                    error: (err) => {
                        this.store.dispatch(ACCOUNT_ACTIONS.clearAccountData());
                        this.logger.log(
                            `⛔ Token: ${type.toUpperCase()} couldn't been refreshed! – ${err.error.statusCode} ${
                                err.error.message
                            }`
                        );
                    },
                    complete: () => {
                        const NEW_DELAY =
                            type === TokenType.ACCESS_TOKEN ? this._accessTokenTimeout : this._oidcTokenTimeout;
                        this.logInfo(type);
    ...
    public logInfo(type: TokenType): void {
        this.logger.log(
            type === TokenType.ACCESS_TOKEN ? `🔑 Access has been token refreshed ` : `👤 ID token has been refreshed `
        );
    }
    ...
    public login(credentials: UserCredentials): Observable<object> {
        ...
        return this.http.post(this.apiLinks.apiLink + 'auth/local', BODY, {
