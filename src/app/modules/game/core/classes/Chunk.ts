...
export class Chunk {
	...
	private _target(mousePos: Point) {
		...
		if (this.tracks.y < 0 && this.tracks.x < 0) {
			...
			this._map.targetedTile = this._map.targetedChunk?._tiles[Constants.chunkSize - 1][
	...
	public renderTileDiagnostics() {
		...
		this._tileOutlineContainer.scale.x = 0.95;
		this._tileOutlineContainer.scale.y = 0.95;

		let origin = new Point(0, 0);
		origin.x += Constants.tileSize * 0.5;

		this._tileOutlineGraphics.lineStyle(1, 0xffffff, 1);
		this._tileOutlineGraphics.beginFill(0xffffff, 0.05);
		this._tileOutlineGraphics.drawPolygon(
			origin,
			new Point(origin.x + Constants.tileSize * 0.5, origin.y + Constants.tileSize * 0.25),
			new Point(origin.x, origin.y + Constants.tileSize * 0.5),
			new Point(origin.x - Constants.tileSize * 0.5, origin.y + Constants.tileSize * 0.25),
		);
