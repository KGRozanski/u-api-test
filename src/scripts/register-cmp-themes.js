...
function getFilesIncludes(files, variant) {
    ...
    files.forEach( name => {
        includesCode += `${variant === 'light' ? "\t" : "\t\t"}` + "@include " + name.split('.')[0].replace('_', '') + `.theme(${variant === 'light' ? '$default-theme' : '$dark-theme'});\r\n`;
    })
...
function insertMixins(code, files) {
    code = code.replace(/(?<=(mat.all-component-themes\(\$default-theme\);)).*(?=\.darkMode)/s, `\r\n${getFilesIncludes(files, "light")}\r\n\t`);
    code = code.replace(/(?<=(mat.all-component-colors\(\$dark-theme\);))(.*?)(?=\})/s, `\r\n${getFilesIncludes(files, "dark")}    `);
    return code;
}